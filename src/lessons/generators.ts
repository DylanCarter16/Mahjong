...
export function genRecognition(difficulty: Difficulty, rng: Rng): LessonItem {
  ...
  return {
    ...
    // but the clock should pressure, not ambush. Early difficulty gets plenty
    // of time (you're still learning to read the glyphs); it tightens only as
    // recognition becomes second nature. The ItemRunner also gates every timed
    // item behind an "I'm ready" tap, so a timer is never sprung on you mid-run.
    timeLimitMs: [12000, 8000, 5000][difficulty - 1],
    exercise: {
      kind: 'choice',
